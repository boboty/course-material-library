...
V1 产品定义以 `docs/product-v1.md` 为唯一产品依据。不得根据页面便利、技术习惯或模型判断自行改变产品语义。
...
* 同客户重复为强提醒，同集团重复为弱提醒，提醒不能阻止继续使用；
* 系统提示不得自动修改素材人工状态；
* 历史效果不得凭推测自动补齐；
* 快速录入只强制标题、类型、正文；
* V1 Markdown 批量导入只负责拆分并生成草稿，不内置 AI 整理；
...
施工 Agent 不得自行将自己完成的 Task 标记为 `PASS`。

### 中途交接

如果 Task 尚未完成，但需要结束当前 Agent 会话、切换模型或把任务交给下一位 Agent，交接前必须先更新 `PROGRESS.md`。
...
下一位 Agent 必须先读取 `AGENTS.md`、`PROGRESS.md` 和当前 Task，再检查现有工作区后继续，不重新从聊天历史推断任务状态。

### 施工结束

执行 Agent 完成施工并通过自验后，在输出最终施工报告之前，必须先更新 `PROGRESS.md`。
...
* 验收通过：标记为 `PASS`，更新 `accepted baseline`；
* 验收发现问题：标记为 `RC`，记录明确待修项；
* RC 修复并完成自验：标记为 `待复验`；
...
只有通过独立验收的 Task 才能标记为 `PASS`，并成为新的 `accepted baseline`。
